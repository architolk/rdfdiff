...
public class Compare {
  ...
  static final String QUERY_PRESENT1 = "SELECT ?s (min(?sl) as ?l) WHERE { GRAPH <urn:unique1> {?s?p?o FILTER (isIRI(?s)) OPTIONAL {?s <http://www.w3.org/2000/01/rdf-schema#label> ?sl}} FILTER NOT EXISTS {GRAPH <urn:model2> {?s?p2?o2}}} GROUP BY ?s";
  static final String QUERY_PRESENT2 = "SELECT ?s (min(?sl) as ?l) WHERE { GRAPH <urn:unique2> {?s?p?o FILTER (isIRI(?s)) OPTIONAL {?s <http://www.w3.org/2000/01/rdf-schema#label> ?sl}} FILTER NOT EXISTS {GRAPH <urn:model1> {?s?p2?o2}}} GROUP BY ?s";
  ...
  private static void printUniques(String name, String querystr) {
    ...
    for (; items.hasNext() ;) {
      ...
      if (row.contains("l")) {
        System.out.print(" \""); System.out.print(row.get("l")); System.out.print("\"");
      }
      System.out.println("");
    }
    System.out.println(".");
